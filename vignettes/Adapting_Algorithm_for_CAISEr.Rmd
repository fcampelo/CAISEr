...
algos <- unique(my.results$data.raw$Algorithm)
ninstances <- length(my.results$instances.sampled)
CIs <- data.frame(instance  = rep(my.results$instances.sampled, times = 2),
                  algorithm = rep(algos, each = ninstances),
                  x.est     = 0, CI.l = 0, CI.u = 0)

for (i in 1:ninstances){
  tmpdata <- my.results$data.raw %>% 
                   filter(Instance == my.results$instances.sampled[i])
...
myplot <- ggplot(CIs, aes(x = instance, 
                          y = x.est, ymin = CI.l, ymax = CI.u,
                          group = algorithm, colour = algorithm,
                          fill = algorithm))
myplot + 
  geom_pointrange(position = position_dodge(width = 0.5), alpha = 0.5) + 
  xlab("Instance") + ylab("IGD") + 
  ggtitle("Estimated mean IGD", subtitle = "for each algorithm on each instance") + 
  theme(legend.position = "bottom",
