...
while (about 10 hours in my Core i7 iMac with 16Gb RAM, using 7 cores). 
...
for (i in seq_along(unique(algopairs))){
  ...
  car::qqPlot(tmp$Phi, 
            pch = 20, las = 1, main = unique(algopairs)[i],
            ylab = "observed", xlab = "theoretical quantiles")
}

```

The normal QQ plots indicate some deviation from normality, but nothing too 
severe. This which gives us confidence in using the t test as our inferential 
procedure of choice (as the sampling distribution of the means will be even more "well-behaved" than the data distribution).

Some preliminary graphical analysis can be quite informative:
```{r, fig.align="center", fig.width=6, fig.height=4}
df <- cbind(my.results$data.summary, Comparison = algopairs)
...
Recall first that `smaller = better` for the quality indicator used in this experiment. Looking at the plot below shows that the differences between all pairs of algorithms were quite high - e.g., differences of up to 150\% in the case of the `Original 2 x MOEAD-DE` comparison. In general the `MOEA/D-DE` seems to generate better (lower) IGD values than both `Original` versions, and `Original 1` seems generally better than `Original 2`. Also, while the variability seem to be reasonably high for all comparisons, they are all generally of the same magnitude, with no large discrepancies in spread.
...
Finally, we can examine our hypothesis tests. If you just want to perform the same tests that were used to design the experiment, the `summary` method of 
class `CAISEr` already outputs the desired information.
