...
#'
#' calc_se(Xk, dif = "percent", type = "all.vs.first", method = "param")
...

{
  ...
  # PAREI AQUI (inside se_param / se_boot)
  if (method == "param"){
    se <- se_param(Xk = Xk, dif = dif, type = type)
  } else if (method == "boot"){
    se <- se_boot(Xk = Xk, dif = dif, type = type,
                  boot.R = boot.R)
  }
