...
#' \code{$name}, with the name of the problem instance function, that is, a
...
#'
#' \code{algorithm} must contain a \code{$name} field (the name
...
#'    See Section \code{Problems and Algorithms} for details.
#' @param se.max desired upper limit for the standard error for the estimated
...
run_nreps2 <- function(instance,         # instance parameters
                       ...
                       ncpus  = 1)       # number of cores to use
{
    ...
    assertthat::assert_that(is.list(algo), length(algo)==2)
    # initial definitions
    n1     <- nstart-1           # initial number of observations
    n2     <- n1
